...
import { Trash2, AlertTriangle, BookOpen, HelpCircle } from 'lucide-react'
...
export default function ExchangeConfigModal({
  ...
  // 安全输入状态
  const [secureInputTarget, setSecureInputTarget] = useState<null | 'hyperliquid' | 'aster'>(null)
            ...
            <>
                      ...
                      {showBinanceGuide && (
                        ...
                        >
                          ...
                          <ol className="list-decimal list-inside space-y-1 mb-3" style={{ paddingLeft: '0.5rem' }}>
                            ...
                            <li>
                              IP 限制选「<strong>无限制</strong>」或添加服务器 IP
                            </li>
                          </ol>
                  ...
                  <div>
                    ...
                    <div className="flex flex-col gap-2">
                      <div className="flex gap-2">
                        ...
                        >
                          {asterPrivateKey ? t('secureInputReenter', language) : t('secureInputButton', language)}
                        </button>
                        {asterPrivateKey && (
                          <button
                            type="button"
              ...
              <div>
                <label className="flex items-center gap-2 text-sm">
                  <input type="checkbox" checked={testnet} onChange={(e) => setTestnet(e.target.checked)} className="form-checkbox rounded" style={{ accentColor: '#F0B90B' }} />
              ...
              >
                <div className="text-sm font-semibold mb-2" style={{ color: '#F0B90B' }}>
                  <span className="inline-flex items-center gap-1">
                    <AlertTriangle className="w-4 h-4" /> {t('securityWarning', language)}
                  </span>
                </div>
                <div className="text-xs space-y-1" style={{ color: '#848E9C' }}>
                  {selectedExchange.id === 'aster' && <div>{t('asterUsdtWarning', language)}</div>}
