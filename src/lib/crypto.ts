...
export class CryptoService {
  ...
  static async fetchPublicKey(): Promise<string> {
    const response = await fetch(`${import.meta.env.VITE_API_BASE}/crypto/public-key`)
    if (!response.ok) {
  ...
  static async decryptSensitiveData(payload: EncryptedPayload): Promise<string> {
    const response = await fetch(`${import.meta.env.VITE_API_BASE}/crypto/decrypt`, {
      method: 'POST',
      headers: {
...
export function diagnoseWebCryptoEnvironment(): WebCryptoEnvironmentInfo {
  ...
  const isLocalhost = hostname ? ['localhost', '127.0.0.1', '::1'].includes(hostname) : false

  const secureContext = typeof window.isSecureContext === 'boolean' ? window.isSecureContext : protocol === 'https:' || (protocol === 'http:' && isLocalhost)
